<script lang='ts'>
  ...
  type OmitProps = 'updateWorldMatrix' | 'resolveReferences' | 'fromJSON' | 'toJSON' | 'sendMessage' | 'lineOfSightTest' | 'handleMessage' | 'setRenderComponent' | 'getWorldPosition' | 'getWorldDirection' | 'rotateTo' | 'lookAt' | 'getDirection' | 'remove' | 'add' | 'update' | 'start' | 'updateOrientation' | 'getSpeed' | 'getSpeedSquared'
